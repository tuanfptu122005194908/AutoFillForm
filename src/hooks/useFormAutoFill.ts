...
export function useFormAutoFill() {
  ...
  const generateResponses = useCallback(async () => {
    ...
    try {
      ...
      setStatus({ current: 0, total: mockResponses.length, status: 'idle', message: `Đã tạo ${mockResponses.length} câu trả lời` });

      toast({
        title: 'Tạo câu trả lời thành công',
        description: `Đã tạo ${mockResponses.length} bộ câu trả lời`,
      });
    } catch (error) {
