...
export function RandomResponseGenerator({ fields, onResponsesReady }: RandomResponseGeneratorProps) {
  ...
  const handleGenerate = () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      for (const field of fields) {
        if ([2, 3, 7].includes(field.type)) {
          // Radio / Dropdown / Grid - pick one based on percentages
          ...
          // Scale / Rating stars - use the detected scale range
          const min = field.scaleMin ?? 1;
          const max = field.scaleMax ?? 5;
          const count = max - min + 1;
          const values = Array.from({ length: count }, (_, i) => String(min + i));
  ...
  const scaleFields = fields.filter(f => f.type === 5 || f.type === 18);
    ...
    <div className="space-y-6">
      ...
      {scaleFields.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            <Percent className="h-4 w-4 text-primary" />
            <Label className="text-sm font-semibold">Tỷ lệ thang đo / đánh giá sao</Label>
          </div>
