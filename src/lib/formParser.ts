...
<<<<<<< HEAD
      ...
      if ([5, 18].includes(type) && entryData[1] && Array.isArray(entryData[1])) {
        // For scales, entryData[1] is an array where each item is [label] or just empty
        // Google Forms scale with 1-5 stars would have entryData[1] = [["1"], ["2"], ["3"], ["4"], ["5"]]
        const scaleOptions: string[] = [];
        
        // Try to extract text labels from the data
        for (let i = 0; i < entryData[1].length; i++) {
          const item = entryData[1][i];
          if (item && Array.isArray(item) && item[0]) {
            // Has a label in array format like ["1"] or ["2"]
            scaleOptions.push(String(item[0]).trim());
          }
        }
        ...
        if (scaleOptions.length > 0) {
          ...
          scaleMin = 1;
          scaleMax = scaleOptions.length;
        } else {
          // Fallback: assume numeric scale based on length
          scaleMin = 1;
